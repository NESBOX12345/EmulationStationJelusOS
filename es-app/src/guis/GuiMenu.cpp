...
GuiMenu::GuiMenu(Window *window, bool animate) : GuiComponent(window), mMenu(window, _("LZ RETRO V-5 2023").c_str()), mVersion(window)
{
	// MAIN MENU
	...
	if (isFullUI)
	{
		...
		addEntry(_("CONFIGURAÇÕES LZ RETRO").c_str(), true, [this] { openGamesSettings_batocera(); }, "iconGames");
...
#ifdef _ENABLEEMUELEC
		addEntry(_("CONFIGURAÇÃO DO SISTEMA LZ").c_str(), true, [this] { openSystemSettings_batocera(); }, "iconSystem");
		//addEntry(_("EMULATIONSTATION SETTINGS").c_str(), true, [this] { openEmuELECSettings(); }, "iconEmuelec");
#endif
		addEntry(_("CONFIGURAR INTERFACE LZ").c_str(), true, [this] { openUISettings(); }, "iconUI");
		addEntry(controllers_settings_label.c_str(), true, [this] { openControllersSettings_batocera(); }, "iconControllers");
		addEntry(_("CONFIGURAR SISTEMA DE SOM").c_str(), true, [this] { openSoundSettings(); }, "iconSound");

		if (ApiSystem::getInstance()->isScriptingSupported(ApiSystem::WIFI)) {
			addEntry(_("CONFIGURAÇÃO DE INTERNET").c_str(), true, [this] { openNetworkSettings_batocera(); }, "iconNetwork");
		...
		if (ApiSystem::getInstance()->isScriptingSupported(ApiSystem::GAMESETTINGS))
			...
			addEntry(_("CONFIGURAÇÃO ENTRADA"), true, [this] { openConfigInput(); }, "iconControllers");
...
//		addEntry(_("UPDATES & DOWNLOADS"), true, [this] { openUpdatesSettings(); }, "iconUpdates");
	...
	else
	{
		...
		addEntry(_("INFORMAÇÃO").c_str(), true, [this] { openSystemInformations_batocera(); }, "iconSystem");
		addEntry(_("DESTRAVAR UI MODE").c_str(), true, [this] { exitKidMode(); }, "iconAdvanced");
	}
